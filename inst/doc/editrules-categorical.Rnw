...
\label{sIntroduction}
Many applications in statistics involve the analyses of categorical data. These
analyses are often hindered by occurrences of incomplete or inconsistent raw
data records. The process of locating and solving such errors is referred to as
{\em data editing}, and it has been estimated that that National Statistics
Institutes spend up to 40\% of their recourses on this process
\citep{waal:2011}. For this reason a lot of attention is paid to the
development of data editing methods which can be automated so that the costs of
statistics production may be reduced. The {\sf R} statistical environment
already has numerous facilities for analyzing categorical data [see
\cite{husson:2010} for an overview]. However, the options for error
...
closing the gap between raw data retrieval and data analyses with {\sf R}. The
paper complements our previous paper \cite{jonge:2011} which discusses the
treatment of numerical data. The algorithms underlying {\sf editrules}'
functionality and the internal representation of categorical data are discussed
as well. The main purpose of the {\sf editrules} package is to provide a
...
functionality for data correction although it does facilitate the
identification of the set of possible values for erroneous fields.
...
%\subsection{Categorical data and edit rules}
