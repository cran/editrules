...
an edit contains variable $k$  if and only if the inner product ${\bf
1}_{d_k}\cdot{\bf a}_k<d_k$, where ${\bf 1}_{d_k}$ is a $d_k$ vector of ones.
...
\label{secErrLocCat}
See the help on {\sf localizeErrors} and {\sf errorLocalizer}.
